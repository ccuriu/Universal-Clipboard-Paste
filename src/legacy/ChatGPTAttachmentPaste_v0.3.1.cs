        ...
        try
        {
            ...
            if(upload!=null)
            {
                ...
                try
                {
                    ...
                    NormalizeModifiersHard();
                    SendKey(VK_RETURN);
                    Log("UPLOAD_KEYBOARD_ENTER");
                    dlg=WaitForFileDialog(900);
                }
                catch(Exception ex)
                {
                    Log("UPLOAD_KEYBOARD_ERROR "+ex.GetType().Name+" "+ex.Message);
                }

                if(dlg==IntPtr.Zero)
                {
                    Log("UPLOAD_KEYBOARD_NO_DIALOG");
